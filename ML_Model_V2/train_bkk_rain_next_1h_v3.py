...
MODEL_NAME = "bkk_rain_next_1h_v3_hist_gradient_boosting"
...
def train(args):
    log(f"Loading training rows from {args.feature_table}...")
    ...
    log(f"Saved model to {output_model_path}")
    log(f"Saved metadata to {output_metadata_path}")
    return metadata


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a V3 Bangkok rain/no-rain model for the next 1 hour."
    )
