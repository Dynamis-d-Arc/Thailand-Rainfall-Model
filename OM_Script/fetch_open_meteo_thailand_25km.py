...
CREATE INDEX IF NOT EXISTS om_thailand_data_forecast_time_idx
  ON {OUTPUT_TABLE} (forecast_time);

CREATE INDEX IF NOT EXISTS om_thailand_data_grid_time_idx
...
def fetch_weather_batch(points, start_date, end_date, timeout, retries, retry_sleep):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            return payload, "open_meteo_historical_weather"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'Fetch one year of Open-Meteo historical hourly data for "Thailand_Grid_25km" '
            'and upsert it into PostgreSQL table "OM_Thailand_Data".'
        )
    )
