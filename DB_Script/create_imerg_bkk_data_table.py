...
CREATE TABLE IF NOT EXISTS {TABLE_NAME} (
    ...
    imerg_longitude double precision NOT NULL,
    imerg_latitude double precision NOT NULL,
    ...
    random_error_mean_mm_h double precision,
    probability_liquid_precipitation_mean_pct double precision,
    precipitation_quality_index_mean double precision,
    ...
    source_service text NOT NULL DEFAULT 'NASA Earthdata Harmony / GES DISC',
    source_granules text[] NOT NULL DEFAULT '{{}}',
    fetched_at timestamp with time zone NOT NULL DEFAULT now(),
    PRIMARY KEY (grid_number, observation_time),
    CONSTRAINT imerg_bkk_grid_fk
        FOREIGN KEY (grid_number) REFERENCES {GRID_TABLE} (grid_number),
    CONSTRAINT imerg_bkk_half_hour_count_check
        CHECK (half_hour_count BETWEEN 0 AND 2),
    CONSTRAINT imerg_bkk_precipitation_check
        CHECK (precipitation_mm IS NULL OR precipitation_mm >= 0),
    CONSTRAINT imerg_bkk_mean_rate_check
        CHECK (precipitation_rate_mean_mm_h IS NULL OR precipitation_rate_mean_mm_h >= 0),
    CONSTRAINT imerg_bkk_max_rate_check
        CHECK (precipitation_rate_max_mm_h IS NULL OR precipitation_rate_max_mm_h >= 0),
    CONSTRAINT imerg_bkk_liquid_probability_check
        CHECK (
            probability_liquid_precipitation_mean_pct IS NULL
            OR probability_liquid_precipitation_mean_pct BETWEEN 0 AND 100
        )
);

CREATE INDEX IF NOT EXISTS imerg_bkk_data_observation_time_idx
    ON {TABLE_NAME} (observation_time);

CREATE INDEX IF NOT EXISTS imerg_bkk_data_local_observation_time_idx
    ON {TABLE_NAME} (local_observation_time);

CREATE INDEX IF NOT EXISTS imerg_bkk_data_grid_time_idx
    ON {TABLE_NAME} (grid_number, observation_time);

CREATE INDEX IF NOT EXISTS imerg_bkk_data_complete_time_idx
    ON {TABLE_NAME} (observation_time, grid_number)
    WHERE is_complete_hour;

CREATE INDEX IF NOT EXISTS imerg_bkk_data_heavy_candidate_idx
    ON {TABLE_NAME} (precipitation_rate_max_mm_h DESC, observation_time)
    WHERE is_complete_hour AND precipitation_rate_max_mm_h > 0;
"""
