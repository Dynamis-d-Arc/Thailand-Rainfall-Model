...
OUTPUT_TABLE = '"OM_Thailand_Forecast_Data"'
...
CREATE INDEX IF NOT EXISTS om_thailand_forecast_data_forecast_time_idx
  ON {OUTPUT_TABLE} (forecast_time);

CREATE INDEX IF NOT EXISTS om_thailand_forecast_data_grid_time_idx
...
def fetch_weather_batch(points, start_date, end_date, timeout, retries, retry_sleep):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            return payload, "open_meteo_historical_forecast_" + FORECAST_MODEL
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'Fetch Open-Meteo HISTORICAL FORECAST (archived ecmwf_ifs forecast runs) for "Thailand_Grid_25km" '
            'and upsert it into PostgreSQL table "OM_Thailand_Forecast_Data".'
        )
    )
