...
CREATE TABLE IF NOT EXISTS {OUTPUT_TABLE} (
    ...
    convective_inhibition double precision,
    pressure_msl double precision,
    model text NOT NULL DEFAULT 'ecmwf_ifs',
    source_api text NOT NULL,
    fetched_at timestamp with time zone NOT NULL DEFAULT now(),
    PRIMARY KEY (grid_number, forecast_time)
);

CREATE INDEX IF NOT EXISTS om_bkk_data_forecast_time_idx
  ON {OUTPUT_TABLE} (forecast_time);

CREATE INDEX IF NOT EXISTS om_bkk_data_grid_time_idx
...
ON CONFLICT (grid_number, forecast_time) DO UPDATE SET
    ...
    convective_inhibition = EXCLUDED.convective_inhibition,
...
def parse_date(value):
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid date: {value}. Use YYYY-MM-DD, for example 2026-07-13."
        ) from exc
...
def date_chunks(start_date, end_date, days_per_chunk):
    ...
    while current <= end_date:
        ...
        if current < today_local < chunk_end:
            yield current, today_local
            current = today_local + timedelta(days=1)
            continue
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if isinstance(payload, dict):
                payload = [payload]
            return payload, "historical_weather"
...
def value_at(hourly, variable, index, length):
...
def warn_missing_requested_variables(payloads, start_date, end_date, source_api):
    missing = []
    undefined_units = []
    for variable in API_HOURLY_VARIABLES:
        all_values = []
        units = set()
        for payload in payloads:
            ...
            if variable in hourly_units:
                units.add(hourly_units[variable])

        if all_values and all(value is None for value in all_values):
            missing.append(variable)
        if "undefined" in units:
            undefined_units.append(variable)

    if missing:
        log(
            f"  Open-Meteo returned only nulls for {', '.join(missing)} "
            f"on {start_date}..{end_date} via {source_api}"
        )
    if undefined_units:
        log(
            f"  Open-Meteo reported undefined units for {', '.join(undefined_units)} "
            f"on {start_date}..{end_date} via {source_api}"
        )
...
def weather_rows(points, payloads, source_api):
    ...
    for point, payload in zip(points, payloads):
        ...
        model = (
            payload.get("model")
            or payload.get("generationtime_model")
            or payload.get("model")
            or "open_meteo_archive"
        )

        for index, time_value in enumerate(times):
            forecast_time, local_forecast_time = parse_open_meteo_local_time(time_value)
            values = [
                value_at(hourly, variable, index, len(times))
                for variable in DB_HOURLY_VARIABLES
            ]
            rows.append(
                (
                    point["grid_number"],
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'Fetch Open-Meteo ECMWF hourly data for coordinates in "Bangkok_Grid_9km" '
            'and upsert it into PostgreSQL table "OM_BKK_DATA".'
        )
    )
    parser.add_argument(
        "--start-date",
        ...
        help="Start date in YYYY-MM-DD format. Defaults to one year before today.",
    )
    parser.add_argument(
        "--end-date",
        type=parse_date,
    ...
    parser.add_argument(
        ...
        type=int,
        default=31,
        help="Date chunk size for each API request.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=10,
        help="Number of grid coordinates per Open-Meteo request.",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="HTTP retry attempts per API request.",
    )
    parser.add_argument(
        "--retry-sleep",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Seconds to wait between successful API requests.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the planned batches without calling Open-Meteo or writing rows.",
    )
    parser.add_argument(
        "--replace",
        action="store_true",
        help='Clear "OM_BKK_DATA" before loading the requested date range.',
    )
    ...
    with connect() as conn:
        ...
        if args.dry_run:
            for start_date, end_date in date_ranges:
                print(f"{start_date}..{end_date}: historical_weather")
            return
        ...
        total_requests = 0
        for start_date, end_date in date_ranges:
            log(f"Fetching {start_date}..{end_date} via historical_weather")
            for batch_index, point_batch in enumerate(point_batches, start=1):
                ...
                warn_missing_requested_variables(
                    payloads,
                    start_date,
                    end_date,
                    source_api,
