"""Build "OM_THAILAND_DATA_PRECOMPUTE" from "OM_Thailand_Data".
...
PANEL_END_LOCAL = "2026-07-19 23:00:00"
...
CREATE INDEX om_thailand_grid_pairs_neighbor_idx
...
DROP TABLE IF EXISTS om_thailand_neighbor_tmp;
CREATE UNLOGGED TABLE om_thailand_neighbor_tmp AS
SELECT
...
ALTER TABLE om_thailand_neighbor_tmp
    ADD CONSTRAINT om_thailand_neighbor_tmp_pk PRIMARY KEY (grid_number, local_forecast_time);
ANALYZE om_thailand_neighbor_tmp;
"""
...
LEFT JOIN om_thailand_neighbor_tmp nf
...
ALTER TABLE {PRECOMPUTE_TABLE}
    ADD CONSTRAINT om_thailand_data_precompute_pk PRIMARY KEY (grid_number, local_forecast_time);

CREATE INDEX om_thailand_data_precompute_local_time_idx
    ON {PRECOMPUTE_TABLE} (local_forecast_time);

CREATE INDEX om_thailand_data_precompute_split_ready_idx
...
def rebuild(conn, unlogged=False, keep_temp=False):
    with conn.cursor() as cur:
        ...
        if not keep_temp:
            cur.execute("DROP TABLE IF EXISTS om_thailand_neighbor_tmp;")
